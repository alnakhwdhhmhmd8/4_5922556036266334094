...
ZelzalDV_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝐆 𝐀 𝐊 - اوامــر المطـور المســاعد](t.me/CU_FG) 𓆪\n\n"
...
async def add_sudo_user(event):
    ...
    output += "**✾╎يتم الان اعـادة تشغيـل بـوت تيبثـون 𝐆 𝐀 𝐊 انتظـر 2-1 دقيقـه ▬▭ ...**"
...
async def _(event):
    ...
    result = "**•❐• قائمــة مـطـورين البــوت الخـاص بـك مـن 𝐆 𝐀 𝐊 :**\n\n"
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str[0] == "آمن":
        ...
        for plugin in input_str:
            if plugin not in PLG_INFO:
                errors += (
                    f"`{plugin}` __There is no such plugin in your CU_FG__.\n"
                )
            else:
    ...
    output = (
        result + "**✾╎يتم الان اعـادة تشغيـل بـوت تيبثـون 𝐆 𝐀 𝐊 انتظـر 2-1 دقيقـه ▬▭ ...**\n"
    )
    if errors != "":
        output += "\n**- خطــأ :**\n" + errors
